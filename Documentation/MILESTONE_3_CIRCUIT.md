...
environment geometry), decision **D-043**, and the CI evidence for the three
...
**Status:** accepted, with a named risk.
...
All three merged with every check green. Label: `automatically validated`.
...
|---|---|---|---|
| 1 | Lap timing and sector splits | Implemented in C++, mirrored in Python, 68 cases pass | `automatically validated` (model only) / `requires local compilation` (C++) |
| 2 | Lap invalidation rules | Implemented and mirrored, first-cause-wins semantics tested | `automatically validated` (model only) / `requires local compilation` (C++) |
...
| 5 | Race test environment geometry | Generator authored, 84 cases pass, layout produced | `automatically validated` (maths) / `requires Blender execution` (mesh) |

The honest summary: the *rules* of a valid lap are specified, mirrored and
tested; the *circuit* is specified and tested arithmetically; and **none of it
has been compiled, imported, or driven.** Milestone 3 becomes complete when a
human with Unreal 5.8 and Blender 5.2 compiles the module, runs the generator
inside Blender, imports the result, and drives a lap that the validator
accepts and a second lap that it correctly rejects.
