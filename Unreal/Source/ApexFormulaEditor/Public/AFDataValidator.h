...
struct APEXFORMULAEDITOR_API FAFValidationIssue
{
	GENERATED_BODY()

	/** Name of the asset the issue was found in. */
	UPROPERTY(BlueprintReadOnly, Category = "ApexFormula|Validation")
	FString AssetName;
...
/**
 * UAFDataValidator - editor-side gate for ApexFormula Data Assets.
 *
 * Architecture reference: TECHNICAL_ARCHITECTURE.md section 5.
 * Decision reference: DECISION_LOG.md D-017, validation is a gate.
 *
 * Every ApexFormula Data Asset implements its own ValidateSelf returning a list
 * of problem strings. This class is the aggregator: it calls ValidateSelf on
 * each asset handed to it and collects the results into one report. The rules
 * themselves deliberately live next to the data they describe, not here, so
 * that runtime code and editor code can never disagree about what is valid.
 *
...
class APEXFORMULAEDITOR_API UAFDataValidator : public UObject
{
	GENERATED_BODY()
