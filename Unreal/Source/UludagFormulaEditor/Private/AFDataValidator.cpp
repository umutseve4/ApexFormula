...
TArray<FAFValidationIssue> UAFDataValidator::ValidateAsset(const UObject* Asset)
{
	...
	else
	{
		...
		Issues.Add(MakeIssue(Asset,
			TEXT("No Uludag Formula validation rule is registered for this asset class")));
	}
...
FString UAFDataValidator::FormatReport(const TArray<FAFValidationIssue>& Issues)
{
	if (Issues.Num() == 0)
	{
		return TEXT("Uludag Formula data validation: 0 issues.");
	}

	TStringBuilder<2048> Builder;
	Builder.Appendf(TEXT("Uludag Formula data validation: %d issue(s)."), Issues.Num());
